...
const routes = {
  ...
  // Get support stats (for admin)
  'GET /api/admin/support-stats': async (req, res) => {
    const adminKey = req.headers['x-admin-key'];
    if (adminKey !== process.env.ADMIN_KEY) {
      res.writeHead(401); 
      return res.end(JSON.stringify({ error: 'Not authorized' }));
    ...
    const averageRating = totalRatings > 0 
      ? (ratedTickets.reduce((sum, t) => sum + t.rating, 0) / totalRatings).toFixed(1)
      : 0;
  ...
  'POST /api/tickets/:id/close': async (req, res, id) => {
    const user = getAuthUser(req);
    
    try {
      const { rating } = await parseBody(req);
      const tickets = readDB('tickets');
      const ticket = tickets.find(t => t.id === id);
      ...
      // Check authorization - either admin or ticket owner
      const adminKey = req.headers['x-admin-key'];
